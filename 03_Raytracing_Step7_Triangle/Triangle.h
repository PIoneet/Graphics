...
namespace hlab
{
	...
	public:
								  ...
								  float &t, float &u, float &v)
		{
			...
			if ( glm::dot(faceNormal, -dir) < 0.0f) return false;

			// 평면과 광선이 수평에 매우 가깝다면 충돌하지 못하는 것으로 판단
			if ( glm::abs(glm::dot(faceNormal, dir)) < 1e-2f) return false; // t 계산시 0으로 나누기 방지

			/* 2. 광선과 평면의 충돌 위치 계산 */
			t = (glm::dot(v0, faceNormal) - glm::dot(orig, faceNormal)) / 
				glm::dot(dir, faceNormal);
			...
			const vec3 normal1 = glm::normalize(glm::cross((point - v2), (v1 - v2)));
