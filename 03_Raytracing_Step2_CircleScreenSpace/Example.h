...
public:
	...
	std::unique_ptr<hlab::Circle> circle1; // Circle의 object
...
public:
	Example(HWND window, int width, int height)
	{
		...
		circle1 = std::make_unique<hlab::Circle>(
			hlab::Circle({ width / 2.0f, height / 2.0f }, height / 4.0f, (height / 4.0f) * (height / 4.0f), 
						 {1.0f, 0.0f, 0.0f, 1.0f} ));
	}
	...
	void Update()
	{
		...
		for (int j = 0; j < height; j++)
			...
			{
				if (circle1->IsInside(glm::vec2{ i, j }))
				{
