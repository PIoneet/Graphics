	...
	public:
		...
		vec3 traceRay(Ray &ray)
		{
			...
			else
			{
				...
				// Specular
				const vec3 reflectDir = 2 * glm::dot(hit.normal, dirToLight) * hit.normal - dirToLight; // r = 2 (n dot l) n - l
				float specular = glm::max( glm::dot( -ray.dir, reflectDir), 0.0f);

				specular = glm::pow( specular, sphere->alpha );
