...
public:
	...
	glm::vec2 TransformScreenToWorld(glm::vec2 positionScreen)
	{
		...
		glm::vec2 worldVector{-aspect + (positionScreen.x / (width-1) ) * (2 * aspect) ,
							1 + -1 * (positionScreen.y / (height - 1) ) * (2 * 1) };
		// -1 + 2t 가 아니라 1 - 2t다.

		// 0 -> height-1 로 가던걸 +1 -> -1 로 가는 걸로 바꿔버려가지고 아예 방향 자체를 바꿔줘야 해서 y는 -1을 마지막에 해줌.
